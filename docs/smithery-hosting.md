...
```bash
npx -y universal-brute-workpack serve --transport streamable-http --host 0.0.0.0 --port 18890 --profile readonly
```
...
npx -y universal-brute-workpack serve --transport streamable-http --host 0.0.0.0 --port "${PORT:-18890}" --profile readonly
```
